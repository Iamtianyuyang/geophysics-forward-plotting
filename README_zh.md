...
![波形显示图](examples/outputs/wiggle.png)
...
| 技能 | 功能 | 使用场景 |
|---|---|---|
| [data-inspector](skills/data-inspector/SKILL.md) | 加载 NumPy 数组并推断常见的 `nt x nx`、`nz x nx`、`nz x ny x nx` 布局 | 形状、布局、物理轴或值域不确定时 |
...
更多三维叠加、任意测线、井曲线、地质体、点云和浏览器渲染功能，请参阅 [CIGVis Gallery](https://cigvis.readthedocs.io/en/latest/gallery/index.html)。适配器接受数组、`.npy` 路径和 YAML 任务配置，不依赖硬编码的 CIGVis 示例路径。
